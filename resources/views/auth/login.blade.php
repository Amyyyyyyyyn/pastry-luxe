...
<section class="mx-auto max-w-md px-4 py-16">
  ...
  <form class="mt-6 space-y-3" method="post" action="{{ route('login.store') }}">@csrf
    <input class="w-full rounded-xl border px-3 py-2" type="email" name="email" placeholder="Email" value="{{ old('email') }}">
    <input class="w-full rounded-xl border px-3 py-2" type="password" name="password" placeholder="Password">
    <label class="flex items-center gap-2 text-sm"><input type="checkbox" name="remember"> Remember me</label>
    <button class="w-full rounded-xl bg-stone-900 px-4 py-3 text-sm font-semibold text-amber-50">Login</button>
  </form>
</section>
@endsection
